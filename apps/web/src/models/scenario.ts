...
} from "./airport-conditions";
import "./airport-info";
import { AirportInfoData } from "./airport-info";
import { Craft, CraftSchema } from "./craft";
...
ScenarioSchema.statics.incrementViews = async function (
  ...
  try {
    ...
    console.error(
      `Error incrementing views for scenario ${scenarioId}:`,
      error,
    );
    throw error;
  }
...
ScenarioSchema.statics.findSummary = async function (
  ...
  try {
    ...
    console.error(`Error finding scenario summaries:`, error);
    throw error;
  }
};
